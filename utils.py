...
async def is_req_subscribed(client, update):
    if not AUTH_CHANNEL:
        return True

    user_id = update.from_user.id
    for group_id in AUTH_CHANNEL:
...
async def check_each(client, group_id, user_id):
    try:
        ...
        join_request_status = await db.find_join_req(user_id, group_id)
                    ...
                    await bot.send_cached_media(
                        ...
                        reply_markup=InlineKeyboardMarkup(
                            [
                                ...
                                InlineKeyboardButton("Bᴏᴛ Oᴡɴᴇʀ", url="t.me/That_roucher")
                                ]
                            ]
                        )
                    )
